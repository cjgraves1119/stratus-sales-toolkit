...
FOR EACH approved action (in order, one at a time):
  ...
  2. Send email via Pipedream (Tier 1, instruction singular), confirm sent
  3. Close task via Zoho CRM, confirm via re-fetch
  4. Check successor enforcement (ALL open/ongoing deals need follow-up)
  5. Create follow-up task (skip ONLY if engagement should genuinely end)
  THEN next task
...
2. Send via Pipedream (Tier 1, instruction singular)
3. Confirm sent before moving to next item
4. Do NOT auto-send without the explicit approval step
...
FOR EACH approved inbox action:
  INBOX_REPLY:
    ...
    2. Send draft via Pipedream (already shown in approval table)
    3. If active deal with no open task: create successor task
