...
description: "6-phase daily task engine with orchestrator-level deal and gmail pre-loading, batched sub-agent launches, lean sub-agent prompts, hybrid email templates, token-optimized architecture, file-piped sub-agent results, deferred companion skill loading, dynamic companion skill versioning, interactive html dashboard, google calendar briefing, parallel sub-agent evaluation, orphaned deal check, batch approval tables, inbox scan, successor enforcement, and centralized voice guide. dashboard: card/compact/kanban views, inline editing, drag-and-drop, dark mode, auto-save, search/filter, batch approve/skip/reject. triggers: daily tasks, review my tasks, task review, task clean up, close out my tasks, close tasks, what tasks are due, fu30s, run my tasks, morning tasks, finish my tasks, knock out my tasks, lets do tasks, whats on my plate, clear my task list, inbox scan, check my inbox, scan my email."
---
