---
name: zoho-crm-v34
description: "zoho crm with stratus ecomm pricing default on all quotes (wooproducts stratus_price, prices.json fallback), auto velocity hub deal approval after did generation, mandatory follow-up task on every new deal (quote creation path), enforced complete payload templates, live batch sku lookup only, master quote workflow, send-quote-to-customer pipeline, gmail thread read pre-step, delinquency gate, and ccw approval shortcut. triggers: create quote, send quote, send quote to customer, new deal, update deal, close task, task review, daily tasks, task clean up, close out my tasks, what tasks are due, review my tasks, submit to ccw, admin action, clone quote, cancel po, generate po and send, deal approval. org id: org647122552."
---
