---
name: zoho-crm-team-v1
description: "team-distributable zoho crm skill with self-serve identity resolution (memory-cached user identity, prompt on first use, /zoho-identity to update), embedded prices.json hot cache for zero-api-call product id + ecomm pricing resolution on all quotes, auto velocity hub deal approval after did generation, mandatory follow-up task on every new deal (quote creation path), enforced complete payload templates, master quote workflow, send-quote-to-customer pipeline, gmail thread read pre-step, delinquency gate, and ccw approval shortcut. triggers: create quote, send quote, send quote to customer, new deal, update deal, close task, task review, daily tasks, task clean up, close out my tasks, what tasks are due, review my tasks, submit to ccw, admin action, clone quote, cancel po, generate po and send, deal approval, /zoho-identity, update my zoho identity. org id: org647122552."
---

# Zoho CRM Team v1 (Identity-Resolution + Embedded Pricing + Product ID Cache)

## IDENTITY RESOLUTION (MANDATORY FIRST STEP)

Before any Zoho write, this skill MUST know who the calling user is. Their identity drives `Owner` assignment, attribution, "show my deals" queries, default fields, and email sign-off. Do NOT proceed to any other section (including MARGIN UPDATE routing) without first confirming `{{USER_ZOHO_ID}}`, `{{USER_FULL_NAME}}`, and `{{USER_EMAIL}}` are populated.

### Step 1: Check memory

Look for `memory/user_zoho_identity.md` in the user's memory directory. Expected format:

```markdown
---
name: user-zoho-identity
description: Logged-in Stratus user identity for Zoho CRM skill — full name, work email, Zoho user ID
metadata:
  type: reference
---

- **Full name:** Tim Pettine
- **Work email:** tim@stratusinfosystems.com
- **Zoho user ID:** 2570562000XXXXXXXXXXX
- **Default Meraki_Team:** MM Heartland-Gulf (id 2570562000347553137)  ← optional
```

If present, extract those values into the session variables:
- `{{USER_FULL_NAME}}` = "Full name" value
- `{{USER_EMAIL}}` = "Work email" value
- `{{USER_ZOHO_ID}}` = "Zoho user ID" value

Done — proceed to the rest of the skill. Use these placeholders' resolved values everywhere the skill template references them.

### Step 2: If memory file missing, prompt for identity ONCE

If `memory/user_zoho_identity.md` doesn't exist, ask the user verbatim:

> "Before I create or update any Zoho records on your behalf, I need to know who you are so attribution flows correctly. Please provide:
> 1. Your full name (as it appears in Zoho)
> 2. Your work email (the one tied to your Zoho user account)
>
> If you know your Zoho user ID, share it. Otherwise I'll look it up from your email."

Once the user provides the info:

1. If the user did not give a Zoho user ID, query Zoho `/users?type=ActiveUsers` and find the user whose `email` matches the provided work email (case-insensitive). Extract the `id` field. If multiple matches or none, report and ask the user to disambiguate.
2. Show the resolved Zoho user ID back to the user for confirmation: `"I found Zoho user 2570562000XXXXXXXXXXX for tim@stratusinfosystems.com — confirm?"`
3. On user confirmation, write `memory/user_zoho_identity.md` with the values (frontmatter + bullets, per the format above).
4. Also add a one-line entry to `memory/MEMORY.md` under the `## Reference` section pointing to the file, e.g. `- [Zoho identity for {{USER_FULL_NAME}}](user_zoho_identity.md) — used by zoho-crm-team-v1 for Owner/attribution on all writes`.

### Step 3: `/zoho-identity` override

If the user types `/zoho-identity`, `update my zoho identity`, `change my zoho identity`, `reset zoho identity`, or any similar phrase, re-run Step 2 and OVERWRITE the existing `memory/user_zoho_identity.md`. Useful when someone switches Zoho accounts, fixes a typo, or a different team member starts using the session.

### Template variables used throughout this skill

Whenever you see one of the following in this SKILL.md (or in `workflows/*.md`, `references/*.md`), substitute the resolved value from the user's identity file at READ time. These are NOT literal strings to send to Zoho — they are placeholders.

| Placeholder | Source | Used for |
|---|---|---|
| `{{USER_ZOHO_ID}}` | Zoho user ID from identity file | `Owner.id` on Deals/Quotes/Tasks; COQL `Owner=` filters; "my tasks" queries |
| `{{USER_FULL_NAME}}` | Full name from identity file | Reference labels, log messages, email sign-off |
| `{{USER_EMAIL}}` | Work email from identity file | Email From, Gmail context |

### Stratus Sales catch-all is NOT user-specific — keep it

The `Stratus Sales` user (id `2570562000027286729`) is the team-wide fallback `Meraki_ISR` for deals where Lead_Source is not "Meraki ISR Referal" (i.e., Stratus Referal, Website, VDC, -None-). It is a shared team account and is NOT the calling user's personal Zoho ID. Do NOT replace `2570562000027286729` with `{{USER_ZOHO_ID}}` anywhere in this skill.

---

## MARGIN UPDATE ROUTING (MANDATORY — TOP PRIORITY)

**STOP if the request matches a margin update phrase. Load `workflows/margin-update.md` BEFORE any calculation.**
...
Why this is enforced at the top:
- Cost basis MUST be `Disti_Price_Total` (pre-rebate) from `Vendor_Lines`, NOT `Net_Price` (post-rebate). Zoho's `Extended_Margin` rollup is calculated against `Disti_Price`. Using `Net_Price` produces math that looks correct (17.5% off Net) but renders as a lower margin (~14%) in Zoho's UI.
- The `Discount` field uses **percentage strings** (e.g. `"17.5%"`), not dollar amounts. See Critical Rule #42.
- Skipping the workflow caused a real production error on a Miniso quote where the displayed margin came out 14% instead of 17.5%.

Workflow file: `workflows/margin-update.md` — load it, follow the formula `Target_Sell = Disti_Price_Total / (1 - margin_decimal)`, then compute and apply the percentage discount per line.
...
```json
{
  "data": [{
    ...
    "Quoted_Items": [
      {"Quantity": 5, "Discount": "67%", "Description": "67% discount applied", "Product_Name": {"id": "product_zoho_id"}},
...
## PRODUCT ID + PRICING LOOKUP (v35 — EMBEDDED CACHE PRIMARY, LIVE API FALLBACK)

**v35 Change:** Embedded `data/prices.json` provides both `zoho_product_id` AND ecomm pricing for 1,026 of 1,042 SKUs. This eliminates the Products module search AND WooProducts pricing lookup for most quotes, saving 2-3 API calls per quote and ~10-15 seconds.

### Step 1: Load Embedded Cache (PRIMARY — ALWAYS USE FIRST)
...
product_id_map = {}  # SKU → zoho_product_id
price_map = {}       # SKU → {list, price, discount}
...
for sku in quote_skus:
    ...
    if price:
        price_map[sku] = {
            'list': entry.get('list', 0),
            'price': price,
            'discount': entry.get('discount', 0.30)
        }
```

**Cache coverage:** 1,026/1,042 SKUs have `zoho_product_id`. 1,042/1,042 have pricing. Only 16 legacy/regional SKUs lack product IDs (Z1 variants, MX50 licenses, etc.).

### Step 2: Live API Fallback (ONLY for cache misses)

If any SKU is missing from the cache or has no `zoho_product_id`:
...
For pricing fallback, check WooProducts:
```
Module: WooProducts
Criteria: (WooProduct_Code:equals:SKU)
Fields: WooProduct_Code,Stratus_Price
```

Filter out bundle records (`+` in WooProduct_Code) from WooProducts results.

### Step 3: If Not Found → Query Unified Product Catalog
...
The embedded `data/prices.json` is refreshed bi-weekly by the `bot-price-refresh-v1-0` scheduled task (woo-sku-discovery). After each refresh, the task updates both the bot repo and this skill's data file. If prices seem stale, run: "refresh bot pricing" to trigger a manual update.
...
### With Discount Applied (v35: Discount = Percentage String)
```json
{
  "Quoted_Items": [
    {"Quantity": 5, "Discount": "67.7%", "Description": "67.7% discount applied", "Product_Name": {"id": "2570562000034080533"}},
    {"Quantity": 1, "Discount": "59.7%", "Description": "59.7% discount applied", "Product_Name": {"id": "2570562000348083785"}}
  ]
}
```

**v35 CRITICAL**: `Discount` is a **percentage string** (e.g. `"67.7%"`), not a number. Formula: `Discount_Percent = ((List_Price × Qty) - Target_Sell) / (List_Price × Qty) × 100`
Example: List $201, Qty 1, target $138 → `"Discount": "31.3%"`
For margin-based pricing, see `workflows/margin-update.md`.
...
## STRATUS PRICING DEFAULT (UPDATED IN V35)
...
Stratus publishes ecomm prices on the website. Quoting at list price creates quotes that don't match what customers see online and require manual correction before sending.

### Price Source Hierarchy (v35)

| Priority | Source | Field | Notes |
|----------|--------|-------|-------|
| 1 (Primary) | Embedded `data/prices.json` (this skill) | `price`, `list`, `discount` | Covers 1,042 SKUs. Refreshed bi-weekly. Zero API calls. |
| 2 (Fallback) | WooProducts module (Zoho CRM) | `Stratus_Price` | Live lookup for any SKU not in cache or with null price |
| 3 (No price found) | List price (no discount) | — | Create at list, flag in description for manual review |

### Step 1: Load Embedded Cache (PRIMARY — ZERO API CALLS)

The cache is already loaded during the PRODUCT ID + PRICING LOOKUP step above. Both `product_id_map` and `price_map` are populated from the same `data/prices.json` read. No additional work needed here if the cache was already loaded.
...
### Step 2: WooProducts Fallback (ONLY for cache misses)

For any SKU where the embedded cache returns no match or `price` is null/0:
...
### Step 3: Calculate Discount (Percentage String)

`Discount` in Zoho is a **percentage string per line** (e.g. `"33%"`), not a dollar amount.

```
Discount_Percent = round((List_Price - Stratus_Price) / List_Price × 100, 1)
Discount_Field = f"{Discount_Percent}%"
```

`List_Price` comes from the embedded cache's `list` field (or from the Zoho Products record if auto-populated). For inline creation, the cache provides both `list` and `price` so the discount can be calculated without any API calls.

Example:
```
MR57-HW: List $3,418.38 | Stratus $2,299 | Qty 7
Discount % = round((3,418.38 - 2,299) / 3,418.38 × 100, 1) = 32.7%
"Discount": "32.7%"
```
  ...
  "Quoted_Items": [
    {
      "Quantity": 7,
      "Discount": "32.7%",
      "Description": "Stratus price $2,299/unit (32.7% off list)",
...
| Discount | "32.7%" | ✓ (percentage string per line) |
```
...
1. Load embedded `data/prices.json` cache (primary, zero API calls)
2. For any cache misses, batch WooProducts lookup (OR-criteria, max 10/call), filter bundles
...
ZohoCRM_Search_Records
Module: Tasks
criteria: (Status:equals:Not Started)and(Owner:equals:{{USER_ZOHO_ID}})
...
42. **DISCOUNT IS PERCENTAGE STRING** - The `Discount` field on Quoted_Items accepts percentage strings (e.g. `"17.5%"`), not decimals or dollar amounts. Formula: `Discount_Percent = ((List_Price × Qty) - Target_Sell) / (List_Price × Qty) × 100`. Example: List $201, Qty 1, target $138 → `"Discount": "31.3%"`. For margin updates, use `Target_Sell = Disti_Price_Total / (1 - margin)` per `workflows/margin-update.md`
...
**Logged-in User ID:** `{{USER_ZOHO_ID}}` (resolved from `memory/user_zoho_identity.md` — see IDENTITY RESOLUTION at top of this file)
**Logged-in User Name:** `{{USER_FULL_NAME}}`
**Logged-in User Email:** `{{USER_EMAIL}}`
**Org ID:** `org647122552`
**Stratus Sales ID (team catch-all, do NOT replace with `{{USER_ZOHO_ID}}`):** `2570562000027286729`
...
✗ Use dollar amount or decimal for Discount — always use percentage string (e.g. `"17.5%"`)
